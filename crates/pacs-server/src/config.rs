...
# The bundled pacsleaf viewer is enabled by default at `/viewer` and extracted
# into `./web/pacsleaf-viewer/dist/` automatically on first start. The bundled
# OHIF viewer stays enabled as a secondary route under `/ohif`.
...
[plugins]
enabled = ["{admin_plugin_id}", "{pacsleaf_plugin_id}", "{viewer_plugin_id}"]
...
[plugins.{pacsleaf_plugin_id}]
static_dir = "./web/pacsleaf-viewer/dist"
route_prefix = "/viewer"
redirect_root = true
index_file = "index.html"
fallback_file = "index.html"
...
"#,
            ...
            pacsleaf_plugin_id = PACSLEAF_VIEWER_PLUGIN_ID,
        )
    }
}
...
mod tests {
    ...
    fn generated_standalone_config_is_valid_and_enables_admin_and_viewer() {
        ...
        assert!(cfg
            .plugins
            .enabled
            .contains(&PACSLEAF_VIEWER_PLUGIN_ID.to_string()));
        assert!(cfg.plugins.configs.contains_key(PACSLEAF_VIEWER_PLUGIN_ID));
        ...
        assert!(toml.contains("[plugins.pacsleaf-viewer]"));
