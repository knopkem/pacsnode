...
///   "version":    "0.1.0",
///   "nodes": [
///     { "ae_title": "MODALITY1", "host": "192.168.1.10", "port": 104, "description": "CT Scanner" }
///   ]
/// }
/// ```
...
mod tests {
    ...
    async fn test_system_info_includes_registered_nodes() {
        use crate::state::DicomNode;
        use std::sync::Arc;

        let mut state = make_test_state(MockMetaStore::new(), MockBlobStr::new());
        *state.nodes.write().await = vec![DicomNode {
